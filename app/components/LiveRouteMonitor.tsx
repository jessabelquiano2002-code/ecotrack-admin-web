...
export function LiveRouteMonitor() {
  ...
  const [navigation, setNavigation] = useState<Record<string, RawRecord>>({});
  ...
  useEffect(() => {
    ...
    const unsubscribers = [
      ...
      listen("driver_navigation", setNavigation),
    ];
    ...
    return allSessions
      .flatMap<DriverActivity>((session) => {
        ...
        return [{
          ...
          assignedCoordinates: (() => {
            const sessionNavigation = normalizeCoordinates({ coordinates: session.data.navigationCoordinates });
            if (sessionNavigation.length >= 2) return sessionNavigation;
            const driverNavigation = asRecords(navigation[selected.driverId]);
            const scheduleNavigation = asRecords(driverNavigation[session.scheduleId]);
            const cachedNavigation = normalizeCoordinates({ coordinates: scheduleNavigation.coordinates });
            return cachedNavigation.length >= 2 ? cachedNavigation : normalizeCoordinates(route);
          })(),
  ...
  }, [selected, allSessions, schedules, routes, drivers, history, navigation]);
      ...
      {selected && <div className="driver-activity-card" id="driver-daily-activity">
        <div className="activity-card-head">
          <div>
            ...
            <h3>{selected.driverName} — Recorded Daily Activity</h3>
            <p>Open a date folder to inspect the driver&apos;s stored GPS trail, barangay coverage, assigned route, and recorded trips.</p>
          </div>
          <button
            type="button"
